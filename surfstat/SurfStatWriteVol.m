...
%step=round(vol.vox./d.vox);%alx removed
step=round(vol.vox./d.vox(1:3));%alx new assuming 3D volx
%s=round((vol.origin-d.origin)./d.vox)+1;%alx removed
 
s=round((vol.origin-d.origin)./d.vox(1:3))+1;%alx new assuming 3D volx
[k,v]=size(data);
for i=1:k
   m(vol.lat)=data(i, :);%alx removed
   
    d.data(s(1)+(0:n(1)-1)*step(1),s(2)+(0:n(2)-1)*step(2), s(3)+(0:n(3)-1)*step(3))=m;
    if isstr(filenames)
