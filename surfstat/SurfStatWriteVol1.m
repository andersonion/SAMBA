...
if Z(1)==1 & T(1)==1
   ...
   % The following code was commented out on 20 December 2016 by BJA.  It is
   % unclear why the hell the existence of the 'vox' field would warrant an
   % [attempted] change in handedness by changing pixdim[0]=-1, which in
   % this case is actually qfac.  Just because.  It does not appear that
   % the data is actually reversed along any of the dimensions.  If so, I
   % will need to find that code and correct that as well.
%    if isfield(d,'vox')
%       pixdim=ones(1,8);
%       pixdim(1)=-1;
...
% Added 20 Dec 2016 by BJA
rm_cmd = ['rm ' d.file_name '.gz'];
gzip_cmd = ['gzip ' d.file_name];

if exist([d.file_name '.gz'],'file')
    system(rm_cmd)
end
system(gzip_cmd);
