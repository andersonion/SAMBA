...
# IMPORTANT: initialize $log_msg so concatenation and logging never complain.
my $log_msg = '';
     ...
     foreach my $V_or_L (@spaces) {
         ...
         foreach my $file (@files_to_check) {
             ...
             if ($case == 1) {
                 ...
                 # Always look for a gzipped output, regardless of input compression.
                 $out_file =~ s/(\.gz)?$/\.gz/;
             } else {
                 $out_file = $file;
             }
...
# ------------------
    ...
    if ($do_registration) {
        ...
        if (compare_two_reference_spaces($in_file,$ref_file)) {
			...
			my ($output_datatype, $dt, $bp, $endian) = ants_output_datatype_for_image($in_file);

            $translation_transform = "${out_path}/translation_xforms/${runno}_0DerivedInitialMovingTranslation.mat";
            $cmd = "antsApplyTransforms -u ${output_datatype} -v ${ants_verbosity} -d ${dims} ${opt_e_string} -i ${in_file} -r ${ref_file}  -n $interp  -o ${out_file} -t ${translation_transform};\n"; 
            @cmds = ($cmd);
        }  
    }
...
# ------------------
    ...
    # CLEANED-UP LOGIC TO AVOID NUMERIC COMPARISONS ON "NO_KEY" / STRINGS
    my $enforcer_is_off = (
        !defined $enforcer ||
        $enforcer eq 'NO_KEY' ||
        $enforcer eq 'UNDEFINED' ||
        $enforcer eq ''
    );
    ...
    if ($enforcer_is_off){
        ...
        my $resample_images_has_key = (
            defined $resample_images &&
            $resample_images ne 'NO_KEY'
        );

        my $resample_factor_has_key = (
            defined $resample_factor &&
            $resample_factor ne 'NO_KEY' &&
            $resample_factor ne 'UNDEFINED_VALUE'
        );

        if ($resample_images_has_key || $resample_factor_has_key) {

            # Case: resample_images explicitly says "no", "off", or 0
            if ($resample_images_has_key && $resample_images =~ /^(no|off|0)$/i) {
                $resample_images = 0;
                $resample_factor = 1;

            # Case: resample_images is a yes/flag (1 or 2 or "yes"/"on")
            } elsif ($resample_images_has_key && $resample_images =~ /^(yes|on|1|2)$/i) {
                $resample_images = 1;
                $resample_factor = 2;

            # Case: resample_images looks numeric (no letters or minus sign) → use as factor
            } elsif ($resample_images_has_key && $resample_images !~ /[\-a-zA-Z]/) {
                $resample_factor = $resample_images;
                $resample_images = 1;

            # Case: only factor set
            } elsif ($resample_factor_has_key && !$resample_images_has_key) {
                $resample_images = 1;
            ...
            # Nothing requested → no resampling
            $resample_images=0;
            $resample_factor=1;
        }

    } else {
        # force_isotropic_resolution set → enforce isotropic
        $resample_images=1;
        $resample_factor='iso';
    }
    ...
    foreach my $V_or_L (@spaces) {    
        ...
        if ($input_reference_path_hash{$V_or_L} eq 'rerun_init_check_later') {
            my $log_msg_local = "Reference spaces not set yet. Will rerun upon start of set_reference_space module.";
            log_info("${message_prefix}${log_msg_local}");
...
# ------------------
    ...
    foreach my $V_or_L (@spaces) {
        ...
        # SAFER REFERENCE CREATION + GUNZIP HANDLING
