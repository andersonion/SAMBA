...
_samba_auto_bind_slurm() {
  ...
  # Bind SAMBA source tree into container so /opt/samba/SAMBA uses host code
  if [[ -d "${SAMBA_APPS_DIR}/SAMBA" ]]; then
    EXTRA_BINDS+=( --bind "${SAMBA_APPS_DIR}/SAMBA:/opt/samba/SAMBA" )
  fi
}
...
samba-pipe() {
  ...
  # Stage HF to BIGGUS_DISKUS for a node-visible, container-visible path
  ...
  # Headfile dir bind (bind directory containing the staged tmp headfile)
