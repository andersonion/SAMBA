...
if isdeployed
    current_code_path=[mfilename('fullpath') '.m'];
    [current_code_path, ~, ~] = fileparts(current_code_path);
    path_parts = strsplit(current_code_path,'/');
    path_parts((end-1):end)=[];
    current_code_path = [strjoin(path_parts,'/') '/'];
else
    current_code_path=[mfilename('fullpath') '.m'];
    [current_code_path, ~, ~] = fileparts(current_code_path);
    current_code_path = [current_code_path '/'];
end
javaaddpath([current_code_path 'poi_library/poi-3.8-20120326.jar']);
javaaddpath([current_code_path 'poi_library/poi-ooxml-3.8-20120326.jar']);
javaaddpath([current_code_path 'poi_library/poi-ooxml-schemas-3.8-20120326.jar']);
javaaddpath([current_code_path 'poi_library/xmlbeans-2.3.0.jar']);
javaaddpath([current_code_path 'poi_library/dom4j-1.6.1.jar']);
javaaddpath([current_code_path 'poi_library/stax-api-1.0.1.jar']);
