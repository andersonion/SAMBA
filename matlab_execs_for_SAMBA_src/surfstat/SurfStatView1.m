...
if isfield(surf,'lat')
    ...
    if ~isstruct(struct)
        %alex
        %T=zeros(dim)+min(struct.data.Data);%alx added and then removed because I run out of memory
        T=zeros(dim); %min value for me is 0 anyway for masks
        %T=zeros(dim)+min(struct(:)); alx removed
        T(surf.lat)=struct;
        T=permute(T,[2 1 3]);
