simple_voxel_based_analysis <- function(dimensionality = 3, imageFileNames = c(), predictors, formula, testType = c("lm", 
 ...
 formula <- as.formula(response ~ diagnosis) # testing to see if this fixes unwanted sign flippage
  ...
  effectValues <- rep(NA, numberOfForegroundVoxels)
  ...
  for (i in 1:numberOfForegroundVoxels) {
    ...
    if (testType == "student.t") {
      ...
      if (inherits(testResults, "try-error")) {
        ...
        effectValues[i] <- testResults$estimate
      }
      ...
      effectValues[i] <- NA # BJ sez: need to fix this, not sure of right answer.
    }
  ...
  antsImageWrite(effectImage, paste(outputPath, "/", filePrefix, "effect.nii.gz", sep = ""))
} 
